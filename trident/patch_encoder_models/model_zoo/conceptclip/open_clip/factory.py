...
from .loss import ClipLoss, DistillClipLoss, CoCaLoss, SigLipLoss, SigLipConceptLoss, SigLipTokenLoss, SigLipConceptLSELoss
...
def load_checkpoint(
    ...
    print(f'Loaded {checkpoint_path} weights into {model.__class__.__name__} model.')
...
def create_model(
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        # if model_cfg.get('init_concept', False) is True:
        #     model_cfg['init_concept'] = model_cfg.pop('concept_loss_weight') > 1e-5
        print(f'model_cfg: {model_cfg}')
        if custom_text:
...
def create_loss(args):
    ...
    elif args.siglipconcept:
        assert not args.horovod, "Horovod not currently supported for SigLip"
        args.siglip = True
        return SigLipConceptLoss(
            rank=args.rank,
            world_size=args.world_size,
        )
    elif args.siglipconceptlse:
        assert not args.horovod, "Horovod not currently supported for SigLip"
        args.siglip = True
        return SigLipConceptLSELoss(
            rank=args.rank,
            world_size=args.world_size,
        )
    elif args.siglip:
    ...
    elif args.sigliptoken:
        assert not args.horovod, "Horovod not currently supported for SigLip"
        args.siglip = True
        return SigLipTokenLoss(
            rank=args.rank,
            world_size=args.world_size,
        )
    return ClipLoss(
